...
var getProxyAgent = (url) => {
  ...
  if (isHttps) {
    ...
    return new import_http_proxy_agent.HttpProxyAgent(proxyUrl);
  }
};
...
var request = async (endpoint, method, headers, data, params, proxyConfig) => {
  ...
  const agent = activeProxy ? getProxyAgent(endpoint) : void 0;
...
var ExchangeOrderBuilderV2 = class {
   ...
   */
  buildOrderHash(orderTypedData) {
    const digest = (0, import_viem3.hashTypedData)(orderTypedData);
    return digest;
  }
};
