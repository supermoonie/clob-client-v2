...
// 设置全局代理
export const setGlobalProxy = (config) => {
    globalProxyConfig = config;
};
// 获取代理agent
const getProxyAgent = (url) => {
    if (!globalProxyConfig || isBrowser)
        return undefined;
...
export const request = async (endpoint, method, headers, data, params, proxyConfig) => {
...
export const post = async (endpoint, options, retryOnError = false) => {
    try {
        const resp = await request(endpoint, POST, options?.headers, options?.data, options?.params, options?.proxyConfig);
        return resp.data;
    }
    catch (err) {
...
export const get = async (endpoint, options) => {
    try {
        const resp = await request(endpoint, GET, options?.headers, options?.data, options?.params, options?.proxyConfig);
        return resp.data;
    }
    catch (err) {
        return errorHandling(err);
...
export const del = async (endpoint, options) => {
    try {
        const resp = await request(endpoint, DELETE, options?.headers, options?.data, options?.params, options?.proxyConfig);
        return resp.data;
    }
    catch (err) {
        return errorHandling(err);
